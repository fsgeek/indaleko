...
def test_deterministic_generation(collection_name: str, num_iterations: int = 3) -> bool:
    ...
    for i, (original_query_id, original_truth_data) in enumerate(all_truth_data):
        ...
        if set(entity_keys) != original_truth_data:
            logger.error(f"Verification FAILED: Truth data is different!")
            logger.error(f"Original: {original_truth_data}")
            logger.error(f"New: {set(entity_keys)}")
            all_verified = False
        else:
            logger.info(f"Verification PASSED: Generated same truth data")
...
def test_cross_run_consistency() -> bool:
    ...
    if metrics1.true_positives != metrics2.true_positives:
        logger.error(f"FAILED: Different number of true positives between runs! {metrics1.true_positives} vs {metrics2.true_positives}")
        return False
    
    if metrics1.false_positives != metrics2.false_positives:
        logger.error(f"FAILED: Different number of false positives between runs! {metrics1.false_positives} vs {metrics2.false_positives}")
        return False
    
    if metrics1.false_negatives != metrics2.false_negatives:
        logger.error(f"FAILED: Different number of false negatives between runs! {metrics1.false_negatives} vs {metrics2.false_negatives}")
        return False
