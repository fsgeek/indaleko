...
def is_rational(instance):
    return hasattr(instance, 'numerator') and hasattr(instance, 'denominator')
        ...
        if exif_info is not None:
            for tag, value in exif_info.items():
                ...
                if isinstance(value, bytes):
                    ...
                    value = '<Image data>'
                elif isinstance(value, tuple) and all(is_rational(x) for x in value):
                    # Convert all rational objects in the tuple to strings
                    value = tuple(rational_to_string(x) if is_rational(x) else x for x in value)
                elif is_rational(value):
                    # Convert individual rational object to string
...
# Then, use this encoder in the write_to_json function:
def write_to_json(data, json_output):
    with open(json_output, 'w', encoding='utf-8') as jsonfile:
        json.dump(data, jsonfile, cls=CustomJSONEncoder, ensure_ascii=False, indent=4)


# Example usage:

# folder_path = '/path/to/your/folder'
json_output = 'output-03.json'
